...
app.get('/', (req, res) => { 
    res.send('<h1 style="color:#00ffcc;background:#121212;height:100vh;text-align:center;padding-top:20%;">🚀 Supreme Master V43 (God-Tier) Active</h1>'); 
});
...
// ============================================================================
const TELEGRAM_TOKEN = '8709803495:AAHo78HVoqB2MPsnhp0wnK5LQAwozzNDHMM'; // 👈 NOTUN TOKEN UPDATED
const OWNER_ID = 5524906942; 
const OWNER_USERNAME = '@Naimish555'; 

const tgBot = new TelegramBot(TELEGRAM_TOKEN, { polling: true });

// 🔥 ULTIMATE GHOST PROTOCOL: Deep Stealth Flags to bypass Meta's AI
...
console.log(`\n🔥 SUPREME MASTER V43 INITIALIZING...\n`);
...
const DIVIDER = '━━━━━━━━━━━━━━━━━━━━';
const FOOTER = `\n${DIVIDER}\n👑 _Supreme System V43_ | Owner: ${OWNER_USERNAME}`;
...
const texts = {
    English: { 
        ...
        statusOnline: "🟢 Engine Online & Synchronized. System is ready.", 
        statusOffline: "🔴 Engine Offline. Authentication required.", 
        statusLabel: "📡 Status"
    },
    Hinglish: { 
        ...
        extract: "Extract Links", approve: "Auto-Approve", rename: "Rename Groups", shield: "Manage GC Msg",
        menuTitle: "🤖 *MAIN COMMAND CENTER*", 
        statusOnline: "🟢 Engine Online & Synchronized. System ready hai.", 
        statusOffline: "🔴 Engine Offline. Number link karein.", 
        statusLabel: "📡 Status"
    },
    Indonesian: { 
        ...
        menuTitle: "🤖 *PUSAT KOMANDO*", 
        statusOnline: "🟢 Mesin Online & Tersinkron. Sistem siap digunakan.", 
        statusOffline: "🔴 Mesin Offline. Otentikasi diperlukan.", 
        statusLabel: "📡 Status"
    }
};
...
function startWhatsAppClient(userId, chatId, cleanNumber) {
    ...
    // 🔥 BAAT KHATAM: Seedha simple logout logic
    client.on('disconnected', async (reason) => { 
        safeSend(chatId, `🚨 *YOUR WA DISCONNECTED*\nReason: ${reason}\n\nSession expired. Phir se login karo!`); 
        
        activeClients.delete(userId); 
        await client.destroy().catch(()=>{}); 
        
        // Ensure multi_session cache is fully wiped so they can login fresh
...
function sendAdminPanel(chatId, userId) {
    ...
    let adminKeyboard = {
        inline_keyboard: [
            ...
            [{ text: '🚫 Ban User', callback_data: 'admin_ban_user' }, { text: '♻️ Unban', callback_data: 'admin_unban_user' }],
...
function sendShieldMenu(chatId, userId, msgId = null) {
    ...
    if (msgId) {
        ...
        safeSend(chatId, txt, { reply_markup: kb });
    }
}
...
tgBot.on('callback_query', async (query) => {
    ...
    if (data === 'menu_logout_confirm') {
        return tgBot.editMessageText(`⚠️ *WARNING: SESSION DISCONNECT*\n\nAre you sure you want to log out? This will completely wipe your current session data from the server so you can link a new WhatsApp number.`, {
            ...
            reply_markup: { inline_keyboard: [ 
                [{ text: '✔️ Yes, Disconnect System', callback_data: 'menu_logout_execute' }], 
    ...
    if (data === 'admin_bot_broadcast') {
        ...
        return tgBot.editMessageText(`📢 *UNIVERSAL BOT BROADCAST*\n\nSend your payload (Text, Image, Video, File) below.\nTotal Target Users: ${knownBotUsers.length}`, { 
            ...
            message_id: query.message.message_id, 
            parse_mode: 'Markdown', 
            reply_markup: { inline_keyboard: [[{text: '❌ Cancel', callback_data: 'btn_admin_panel'}]] } 
        });
    }
    ...
    if (data === 'sec_tgt_select') {
        ...
        try {
            ...
            if (state.adminGroups.length === 0) {
                return safeSend(chatId, "❌ Admin rights not found in any group.");
            }
            ...
            return tgBot.editMessageText('🎯 *SELECT TARGETS:*', { 
                chat_id: chatId, 
                message_id: query.message.message_id, 
                parse_mode: 'Markdown', 
                reply_markup: getPaginationKeyboard(userId) 
            });
        } catch(e) {}
    }
    ...
    if (data === 'menu_login') { 
        ...
        const msgTxt = `🔐 *SECURE AUTHENTICATION*\n${DIVIDER}\nPlease enter your WhatsApp Number with the Country Code, but **WITHOUT** the '+' sign.\n\n✅ Correct Format: \`919876543210\`\n❌ Wrong Format: \`+919876543210\`\n❌ Wrong Format: \`9876543210\``;
        return tgBot.editMessageText(msgTxt, { 
            chat_id: chatId, 
            message_id: query.message.message_id, 
            parse_mode: 'Markdown', 
            reply_markup: { inline_keyboard: [[{ text: '❌ Cancel Login', callback_data: 'btn_main_menu' }]] } 
        });
    }
    ...
    if (data.startsWith('admin_')) {
        if (data === 'admin_allow_user') { 
            ...
            return tgBot.editMessageText(`✅ Provide ID to allow:`, { chat_id: chatId, message_id: query.message.message_id, reply_markup: { inline_keyboard: [[{text: '❌ Cancel', callback_data: 'btn_admin_panel'}]] } }); 
        }
        
        if (data === 'admin_revoke_user') { 
            state.action = 'WAITING_FOR_REVOKE_ID'; 
            return tgBot.editMessageText(`❌ Provide ID to revoke:`, { chat_id: chatId, message_id: query.message.message_id, reply_markup: { inline_keyboard: [[{text: '❌ Cancel', callback_data: 'btn_admin_panel'}]] } }); 
        }
        
        if (data === 'admin_ban_user') { 
            state.action = 'WAITING_FOR_BAN_ID'; 
            return tgBot.editMessageText(`🚫 Provide ID to ban:`, { chat_id: chatId, message_id: query.message.message_id, reply_markup: { inline_keyboard: [[{text: '❌ Cancel', callback_data: 'btn_admin_panel'}]] } }); 
        }
        
        if (data === 'admin_unban_user') { 
            state.action = 'WAITING_FOR_UNBAN_ID'; 
            return tgBot.editMessageText(`♻️ Provide ID to unban:`, { chat_id: chatId, message_id: query.message.message_id, reply_markup: { inline_keyboard: [[{text: '❌ Cancel', callback_data: 'btn_admin_panel'}]] } }); 
        }
        ...
        if (data === 'admin_toggle_fsub') { 
            adminConfig.fsubEnabled = !adminConfig.fsubEnabled; 
            saveAdminConfig(); 
            return sendAdminPanel(chatId, userId); 
        }
        
        if (data === 'admin_toggle_approval') { 
            adminConfig.approvalRequired = !adminConfig.approvalRequired; 
            saveAdminConfig(); 
            return sendAdminPanel(chatId, userId); 
        }
        
        if (data === 'admin_toggle_alerts') { 
            adminConfig.botAlerts = !adminConfig.botAlerts; 
            saveAdminConfig(); 
            return sendAdminPanel(chatId, userId); 
        }
    ...
    if (data.startsWith('perm_feat_')) {
        if (userId !== OWNER_ID) {
            return tgBot.answerCallbackQuery(query.id, { text: '⚠️ Owner Only!', show_alert: true });
        }
    ...
    if (data.startsWith('rem_')) {
        if (userId !== OWNER_ID) {
            return tgBot.answerCallbackQuery(query.id, { text: 'Owner Only!', show_alert: true });
        }
    ...
    if (menuActions.includes(data)) {
        ...
        try {
            ...
            if (state.adminGroups.length === 0) {
                return safeSend(chatId, "❌ Admin rights not found.");
            }
...
function sendGroupSettingsMenu(chatId, userId, msgId) {
    ...
    if (msgId) {
        ...
        safeSend(chatId, `⚙️ *Phase 6: Permissions*`, { reply_markup: kb });
    }
}
...
tgBot.on('message', async (msg) => {
    ...
    if (state.action === 'WAIT_BOT_BROADCAST_MSG') {
        ...
        if (targets.length === 0) {
            return safeSend(chatId, `⚠️ No bot users in database.`);
        }
    ...
    if (state.action && state.action.startsWith('WAIT_SEC_')) {
        if (state.action === 'WAIT_SEC_LINKS') {
            ...
            if(getSecurityConfig(userId).targetMode !== 'LINKS') {
                getSecurityConfig(userId).targetGroups = [];
            }
            
            // 🔥 GHOST PROTOCOL: Anti-Ban Random Join Delays
            for (let code of codes) {
                try {
                    ...
                    if (!getSecurityConfig(userId).targetGroups.includes(gid)) {
                        getSecurityConfig(userId).targetGroups.push(gid);
                    }
    ...
    if (state.action === 'WAITING_FOR_ADMIN_ID') {
        ...
        if (!isNaN(newAdmin) && !adminConfig.admins.includes(newAdmin)) {
            ...
            saveAdminConfig();
            safeSend(newAdmin, `👑 *ACCESS LEVEL UPGRADED*\n${DIVIDER}\nHello. You are now an Admin of this system.\n\nThe Owner has authorized your account with advanced administrative privileges. You can now manage features, groups, and security settings.\n\nTap or send /start to initialize your Command Center.`);
        }
        state.action = null; 
        return safeSend(chatId, `👑 Admin rights granted to \`${newAdmin}\` and notification dispatched.`);
    }
    ...
    if (state.action === 'WAITING_FOR_ALLOW_ID') { 
        ...
        saveAdminConfig(); 
        state.action = null; 
        return safeSend(chatId, `✅ User successfully allowed.`); 
    }
    ...
    if (state.action === 'WAITING_FOR_REVOKE_ID') { 
        ...
        saveAdminConfig(); 
        state.action = null; 
        return safeSend(chatId, `❌ Access successfully revoked.`); 
    }
    ...
    if (state.action === 'WAITING_FOR_BAN_ID') { 
        ...
        saveAdminConfig(); 
        state.action = null; 
        return safeSend(chatId, `🚫 User successfully banned.`); 
    }
    ...
    if (state.action === 'WAITING_FOR_UNBAN_ID') { 
        ...
        saveAdminConfig(); 
        state.action = null; 
        return safeSend(chatId, `♻️ User successfully unbanned.`); 
    }
    ...
    if (state.action === 'WAIT_GROUP_NAME') { 
        ...
        return safeSend(chatId, `🔢 *Phase 2:* Quantity?`, { reply_markup: { inline_keyboard: [[{text: '❌ Cancel', callback_data: 'btn_main_menu'}]] } }); 
    } 
    
    if (state.action === 'WAIT_GROUP_COUNT') { 
        state.groupConfig.count = parseInt(text); 
        state.action = 'WAIT_GROUP_MEMBER'; 
        return safeSend(chatId, `👤 *Phase 3:* Member ID?`, { reply_markup: { inline_keyboard: [[{text: '❌ Cancel', callback_data: 'btn_main_menu'}]] } }); 
    } 
    
    if (state.action === 'WAIT_GROUP_MEMBER') { 
        state.groupConfig.memberId = text.replace(/[^0-9]/g, '') + '@c.us'; 
        state.action = 'WAIT_GROUP_DESC'; 
        return safeSend(chatId, `📝 *Phase 4:* Desc?`, { reply_markup: { inline_keyboard: [[{text: '⏩ Skip', callback_data: 'grp_skip_desc'}], [{text: '❌ Cancel', callback_data: 'btn_main_menu'}]] } }); 
    } 
    
    if (state.action === 'WAIT_GROUP_DESC') { 
        state.groupConfig.desc = text; 
        state.action = 'WAIT_GROUP_PFP'; 
        return safeSend(chatId, `🖼️ *Phase 5:* DP?`, { reply_markup: { inline_keyboard: [[{text: '⏩ Skip', callback_data: 'grp_skip_pfp'}], [{text: '❌ Cancel', callback_data: 'btn_main_menu'}]] } }); 
    }
    ...
    if (state.action === 'WAIT_BROADCAST_MSG') {
        ...
        let statusMsg = await safeSend(chatId, `⏳ *Transmitting WA Broadcast...*`);
        let mediaObj = null; 
        ...
        try {
            ...
            for (let t of targets) { 
                try { 
                    ...
                    // 🔥 GHOST PROTOCOL: Dynamic typing duration based on message length!
                ...
                // 🔥 GHOST PROTOCOL: Jittered heavy delay between messages
    ...
    if (state.action === 'WAIT_JOIN_LINKS') {
        ...
        for (let i = 0; i < codes.length; i++) {
            try { 
                ...
                // 🔥 GHOST PROTOCOL: Long random delay for joins
    ...
    try {
        for (let i = 1; i <= config.count; i++) {
            ...
            try {
                ...
                // 🔥 GHOST PROTOCOL: Heavy delay to prevent mass creation ban
...
tgBot.on('polling_error', (error) => {
    if (error.code === 'ETELEGRAM' && error.message.includes('409 Conflict')) {
        console.log('\n[🚨 ANTI-CRASH ALERT] 409 CONFLICT: Token is active on another server! Please kill other instances. bkc!');
    } else {
